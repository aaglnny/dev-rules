...
- 业务成员和参数声明明确类型；有返回值的函数声明返回类型，复杂局部值也应补充类型。
...
- 箭头函数的参数类型应明确；有返回值时显式声明返回类型，没有返回值时可以省略 `: void`。
- `ForEach`、`map`、`filter`、监听器和异步回调中的参数类型要明确；有返回值时声明返回类型，没有返回值时可以省略 `void`。
...
- 异步函数有返回结果时显式声明 `Promise<T>`；没有返回结果时可以省略 `Promise<void>`，由编译器推断返回类型。
...
- [ ] 有返回值的参数、返回值和回调类型明确；无返回值的箭头函数和异步函数可以省略 `void` 或 `Promise<void>`。
- [ ] 没有解构声明和不受支持的动态语法。
