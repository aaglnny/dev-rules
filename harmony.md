...
- 导航 API：主框架内部页面跳转统一优先使用 pathStack.pushPath({ name, param })；需要返回回调时，优先使用 pathStack.pushPath({ name, param, onPop })
...
| onPop 回调不触发 | 需要返回回调时，优先使用 pathStack.pushPath({ name, param, onPop }) |
...
export struct ChildPage {
  ...
  private navigateToDetail(): void {
    const param: Record<string, Object> = { 'id': 123 }
    this.pathStack?.pushPath({ name: 'DetailPage', param })
  }
}
```
...
**注意事项**
- 主框架内部页面跳转统一优先使用 `pathStack.pushPath({ name, param })`
- 跳转参数统一放入 `param` 字段，优先使用 `Record<string, Object>` 定义参数对象
- 需要返回回调时，优先使用 `pathStack.pushPath({ name, param, onPop })`
...
### 构建约束
- [ ] 除非用户明确要求，不主动执行编译、构建、打包任务（如 `assembleApp`、`assembleDebug`、`hvigorw` 等）。如需验证，仅做静态检查或说明建议执行的命令。
